...
import { BellRing, FileCheck, Calendar, UserCog } from "lucide-react";
...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1QJzboxloPluqZg15GWL3Y13ymUw_iCydxmpY7Njqtb8";
      ...
      // Split by comma and trim each page, then filter out empty strings
      ...
      // Remove duplicates and sort
      setDepartmentOptions(filteredDepartments.sort());
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1QJzboxloPluqZg15GWL3Y13ymUw_iCydxmpY7Njqtb8/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1QJzboxloPluqZg15GWL3Y13ymUw_iCydxmpY7Njqtb8";
    ...
    if (formData.frequency === "one-time") {
      ...
      while (currentIndex < futureDates.length) {
        ...
        tasks.push({
          ...
          enableReminders: formData.enableReminders,
          requireAttachment: formData.requireAttachment,
          adminReminder: formData.adminReminder,
        });
              ...
              <div className="space-y-4 pt-2 border-t border-purple-100">
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <label
                      htmlFor="admin-reminder"
                      className="text-purple-700 font-medium"
                    >
                      Admin Reminder
                    </label>
                    <p className="text-sm text-purple-600">
                      Send reminders to admin when task is completed
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        id="admin-reminder"
                        checked={formData.adminReminder}
                        onChange={(e) => handleSwitchChange("adminReminder", e)}
