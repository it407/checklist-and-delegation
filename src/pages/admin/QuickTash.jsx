...
export default function QuickTask() {
    ...
    const submitSelectedTasks = async () => {
        try {
            ...
            const userAppScriptUrl = "https://script.google.com/macros/s/AKfycbwhk-Y25IZbYn9V3hfhf3c7WvJ0v9GIDuWDBpo-YCN3gumep3h5USTFw_86cHIZ2aUs/exec";
    ...
    const updateExistingTasks = async () => {
        try {
            ...
            // Prepare tasks for update - use existing Task ID and row index
            const tasksToUpdate = Array.from(selectedRows).map(taskId => {
                const editedTask = editedData[taskId];
                const originalTask = tasks.find(t => t._id === taskId);

                // Use original Task ID and row index
                return {
                    rowIndex: originalTask._rowIndex, // This is the actual row number in sheet
                    taskId: originalTask['Task ID'] || '', // Original Task ID
                    department: editedTask.Department || "",
                    givenBy: editedTask['Given By'] || "",
                    name: editedTask.Name || "",
                    description: editedTask['Task Description'] || "",
                    startDate: formatDateForSheet(editedTask['End Date']),
                    freq: editedTask.Frequency || "",
                    enableReminders: editedTask.Reminders || "",
                    requireAttachment: editedTask.Attachment || ""
                };
            ...
            // Call the new updateChecklistTasks function in Apps Script
            const updateUrl = `${userAppScriptUrl}?action=updateChecklistTasks`;
            const response = await fetch(updateUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({
                    sheetName: CONFIG.CHECKLIST_SHEET,
                    action: 'updateChecklistTasks',
                    rowData: JSON.stringify(tasksToUpdate)
                })
            });
            ...
            alert(`Successfully updated ${tasksToUpdate.length} task${tasksToUpdate.length === 1 ? '' : 's'}!`);
    ...
    const CONFIG = {
        SHEET_ID: "1QJzboxloPluqZg15GWL3Y13ymUw_iCydxmpY7Njqtb8",
        WHATSAPP_SHEET: "master", // For login credentials and user roles
        CHECKLIST_SHEET: "All Checklist Unique", // For unique checklist tasks
                ...
                <>
                    {activeTab === 'checklist' ? (
                        <div className="mt-4 rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
                            ...
                            {selectedRows.size > 0 && (
                                <div className="mb-4 flex justify-between p-4 bg-blue-50 border-b">
                                    <div className="flex items-center gap-4">
                                        ...
                                        {/* <button
        onClick={() => {
          // Check if we're editing existing tasks
          const isEditingExisting = Array.from(selectedRows).some(taskId => {
            const task = tasks.find(t => t._id === taskId);
            return task && task['Task ID'] && task['Task ID'].trim() !== '';
          });
          
          if (isEditingExisting) {
            if (confirm("These tasks already exist. Do you want to UPDATE the existing tasks instead of creating new ones?")) {
              updateExistingTasks();
            } else {
              submitSelectedTasks();
            }
      ...
      >
        {submitting ? (
          ...
          `Submit ${selectedRows.size} Selected Task${selectedRows.size === 1 ? '' : 's'}`
                                    ...
                                    {/* Optional: Add a separate Update button if you want */}
                                    <button
                                        onClick={updateExistingTasks}
                                        disabled={submitting}
